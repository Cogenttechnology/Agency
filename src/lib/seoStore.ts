/* ─────────────────────────────────────────────────────────
   SEO Store — per-page meta + schema, localStorage-based
   ───────────────────────────────────────────────────────── */

export interface PageSchema {
  type: 'Organization' | 'LocalBusiness' | 'WebPage' | 'Service' | 'Article' | 'FAQPage' | 'BreadcrumbList';
  enabled: boolean;
  custom: string; // raw JSON-LD string (editable)
}

export interface PageSeo {
  id: string;            // matches route key e.g. 'home', 'about'
  label: string;         // display name e.g. 'Home'
  path: string;          // route path e.g. '/'
  title: string;         // <title>
  metaTitle: string;     // og:title / twitter:title
...
const DEFAULT_PAGES: PageSeo[] = [
  ...
  },
  {
    ...
    metaDescription: 'Meet the team behind Cogent Agency. 7+ years of experience, 200+ brands served, and a relentless obsession with results.',
  ...
  },
  {
    ...
    schemas: [
      { type: 'Service', enabled: true, custom: serviceSchema('Performance Marketing', 'ROI-driven paid media campaigns across Google, Meta, and YouTube.') },
    ],
    updatedAt: new Date().toISOString(),
  },
  {
    ...
    schemas: [
      { type: 'Service', enabled: true, custom: serviceSchema('SEO Services', 'Data-driven SEO including technical SEO, content strategy, and link building.') },
    ],
    updatedAt: new Date().toISOString(),
  },
  {
    ...
    schemas: [
      { type: 'Service', enabled: true, custom: serviceSchema('Social Media Marketing', 'Strategic content, community management, and paid social campaigns.') },
    ],
    updatedAt: new Date().toISOString(),
  },
  {
    ...
    schemas: [
      { type: 'Service', enabled: true, custom: serviceSchema('Video Production', 'Brand films, reels, ads and explainer videos produced in our studio.') },
    ],
    updatedAt: new Date().toISOString(),
  },
  {
    ...
    schemas: [
      { type: 'Service', enabled: true, custom: serviceSchema('Influencer Marketing', 'End-to-end creator partnerships from nano to mega influencers.') },
    ],
    updatedAt: new Date().toISOString(),
  },
  {
    ...
    schemas: [
      { type: 'Service', enabled: true, custom: serviceSchema('Web Design & Development', 'High-performance React and Next.js websites built to convert.') },
    ],
    updatedAt: new Date().toISOString(),
  },
  {
    ...
    schemas: [
      { type: 'Service', enabled: true, custom: serviceSchema('360° Digital Marketing', 'Full-funnel integrated digital marketing across every channel.') },
    ],
    updatedAt: new Date().toISOString(),
  },
  {
  ...
  },
  {
    ...
    label: 'Portfolio',
    path: '/portfolio',
    title: 'Our Work | Real Results for Real Clients — Cogent Agency',
    metaTitle: 'Our Work & Client Results | Cogent Agency Portfolio',
    metaDescription: 'See how Cogent Agency helped clients rank #1 on Google — from best urologist in Jaipur and best oncologist in Jaipur to event planners, jewellers, hospitals, and D2C brands. Real SEO results, real growth.',
    ogImage: 'https://cogent.agency/og-portfolio.jpg',
    canonical: 'https://cogent.agency/portfolio',
    robots: 'index, follow',
    keywords: 'digital marketing portfolio, SEO case studies, best urologist in jaipur, best oncologist in jaipur, best multispeciality hospital in jaipur, event planner in jaipur, best jewellers in jaipur, financial consulting companies in india, bhaji boxes in delhi, digital marketing agency in jaipur, website development company in jaipur, social media marketing company in jaipur',
    schemas: [
      { type: 'WebPage', enabled: true, custom: webPageSchema('Our Work & Client Results — Cogent Agency', 'https://cogent.agency/portfolio', 'Cogent Agency has helped clients across healthcare, hospitality, fashion, finance, and FMCG rank on page 1 of Google. Explore real SEO results and case studies.') },
    ],
    updatedAt: new Date().toISOString(),
  },
  {
    ...
    schemas: [
      { type: 'WebPage', enabled: true, custom: webPageSchema('Blog', 'https://cogent.agency/blog', 'Digital marketing insights and tips.') },
    ],
    updatedAt: new Date().toISOString(),
  },
  {
...
export function getAllSeoPages(): PageSeo[] {
  ...
  try {
    ...
    if (!stored) {
      window.localStorage.setItem(KEY, JSON.stringify(DEFAULT_PAGES));
      return DEFAULT_PAGES;
    }
    const parsed: PageSeo[] = JSON.parse(stored);
    const storedIds = new Set(parsed.map(p => p.id));
    const merged = [
      ...parsed,
      ...DEFAULT_PAGES.filter(d => !storedIds.has(d.id)),
    ];
    return merged;
  } catch {
    return DEFAULT_PAGES;
...
export function getSeoByPath(path: string): PageSeo | null {
...
export function saveSeoPage(page: PageSeo): void {
  ...
  window.dispatchEvent(new Event('cogent_seo_updated'));
}
...
export function resetSeoPage(id: string): void {
  ...
  if (idx >= 0) all[idx] = { ...def, updatedAt: new Date().toISOString() };
  window.localStorage.setItem(KEY, JSON.stringify(all));
}
